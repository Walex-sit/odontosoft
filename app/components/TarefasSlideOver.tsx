...
export default function TarefasSlideOver({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex justify-end">
      ...
      {/* Painel Lateral */}
      <div 
        className={`relative w-full max-w-sm h-full bg-slate-50 dark:bg-slate-950 flex flex-col shadow-2xl transition-transform duration-300 ease-out transform ${
      ...
      >
        ...
        <div className="p-6 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Tarefas do Dia</h2>
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mt-0.5">{pendentes} {pendentes === 1 ? 'pendente' : 'pendentes'}</p>
              </div>
            </div>
            <button 
              onClick={handleClose}
              className="p-2 text-slate-400 hover:text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-colors"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {tarefas.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Você não tem tarefas pendentes para hoje.</p>
            </div>
          ) : (
              ...
              {tarefas.map(tarefa => (
                <div 
                  ...
                  className={`p-4 rounded-2xl border transition-all ${
                    tarefa.completed 
                      ? 'bg-slate-50 dark:bg-slate-950 border-slate-200 dark:border-slate-700 opacity-60' 
                      : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 shadow-sm hover:border-blue-300 hover:shadow-md cursor-pointer'
                  }`}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-2">
                        <h4 className={`text-sm font-bold ${tarefa.completed ? 'text-slate-500 dark:text-slate-400 line-through' : 'text-slate-800 dark:text-slate-100'}`}>
                          {tarefa.title}
                        </h4>
                      ...
                      <p className={`text-xs mt-1 ${tarefa.completed ? 'text-slate-400' : 'text-slate-500 dark:text-slate-400'}`}>
                        {tarefa.description}
                      </p>
        ...
        {/* Footer */}
        <div className="p-6 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 shrink-0">
          <button 
            className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:text-slate-200 rounded-xl font-bold text-sm transition-colors"
            onClick={handleClose}
          >
            Fechar Painel
          </button>
