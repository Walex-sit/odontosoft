...
export default function Topbar() {
  ...
  return (
    <header className="h-16 w-full flex items-center justify-between px-6 z-50 bg-white border-b border-slate-200 shadow-sm shrink-0">
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <button 
          onClick={handleBusca}
          className="hidden md:flex items-center gap-3 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-xl text-slate-400 transition-colors mr-2 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
        >
          <Search className="h-4 w-4" />
          <span className="text-sm font-medium">Buscar...</span>
          <kbd className="hidden sm:inline-flex items-center gap-1 bg-white border border-slate-200 rounded px-1.5 py-0.5 text-[10px] font-bold text-slate-500 shadow-sm">
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative">
            <button 
              ...
              className="p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-800 rounded-xl transition-colors relative"
              title="Notificações"
            >
          ...
          <Link href="/configuracoes" className="p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-800 rounded-xl transition-colors" title="Configurações">
        ...
        <div className="relative ml-2">
          <button 
            ...
            className="flex items-center gap-2 p-1 pl-2 pr-1 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/20"
          >
            <div className="flex flex-col text-right hidden sm:flex">
              <span className="text-xs font-bold text-slate-800 leading-none">{profile?.nome || 'Usuário'}</span>
            </div>
          ...
          {isUserMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2">
              <div className="px-4 py-3 border-b border-slate-100 mb-1">
                ...
                <p className="text-xs font-medium text-slate-500 truncate">{session?.user?.email || 'testealex@gmail.com'}</p>
              </div>
              <Link href="/minha-conta" className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-blue-600 font-medium transition-colors">
                <User className="h-4 w-4" /> Meu Perfil
              </Link>
              <Link href="/establishment/subscription" className="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:text-blue-600 font-medium transition-colors">
