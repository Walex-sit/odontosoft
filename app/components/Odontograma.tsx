...
function DenteVisual({ numero, status, onClick }: DenteProps) {
  const cores = {
    saudavel: 'bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300',
    carie: 'bg-red-50 border-red-400 text-red-600',
    restaurado: 'bg-blue-50 border-blue-400 text-blue-600',
    ausente: 'bg-slate-100 border-slate-300 dark:border-slate-700 text-slate-400 opacity-50',
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Odontograma</h3>
        <div className="flex items-center bg-slate-100 p-1 rounded-xl">
          <button
            onClick={() => setTipo('permanente')}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${tipo === 'permanente' ? 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'}`}
          >
            Permanentes
          </button>
          <button
            onClick={() => setTipo('deciduo')}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${tipo === 'deciduo' ? 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-200'}`}
          >
            Decíduos
          </button>
        </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-[inset_0_2px_10px_rgba(0,0,0,0.02)] flex flex-col gap-8 items-center overflow-x-auto">
        {/* Superior */}
        ...
        <div className="w-full max-w-2xl h-px bg-slate-200 relative flex justify-center">
          <span className="absolute -top-3 bg-slate-50 dark:bg-slate-950 px-2 text-xs font-bold tracking-wider text-slate-400 uppercase">Direita | Esquerda</span>
        </div>
      ...
      {selectedDente && (
        <div className="p-4 bg-white dark:bg-slate-800 border border-blue-100 rounded-xl shadow-sm animate-in fade-in slide-in-from-bottom-2 flex items-center justify-between gap-4">
          <p className="font-semibold text-slate-800 dark:text-slate-100 text-sm">Alterar status do dente <span className="text-blue-600">{selectedDente}</span>:</p>
          <div className="flex flex-wrap gap-2">
            <button onClick={() => changeStatus('saudavel')} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 dark:text-slate-200 text-xs font-bold rounded-lg transition-colors">Saudável</button>
            ...
            <button onClick={() => changeStatus('ausente')} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-500 dark:text-slate-400 border border-slate-300 dark:border-slate-700 text-xs font-bold rounded-lg transition-colors">Ausente</button>
