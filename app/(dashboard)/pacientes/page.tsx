...
function ErrorCard({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="m-6 bg-red-500/5 border border-red-500/20 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
      ...
      <button
        onClick={onRetry}
        className="flex items-center gap-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:text-slate-100 dark:text-white bg-slate-700 hover:bg-slate-600 px-4 py-2 rounded-xl transition-all border border-slate-600 shrink-0 active:scale-95"
      >
...
function EmptyState({ filtered }: { filtered: boolean }) {
  return (
    <div className="text-center py-16 px-4">
      <div className="h-16 w-16 bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-400 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-200 dark:border-slate-700 dark:border-slate-800">
        <Users className="h-8 w-8" />
      </div>
      {filtered ? (
        <>
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">Nenhum resultado encontrado</h3>
          <p className="text-slate-500 text-sm">Tente buscar por outro nome.</p>
        </>
      ) : (
        <>
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-1">Nenhum paciente cadastrado</h3>
          <p className="text-slate-500 text-sm">Adicione seu primeiro paciente usando o formulário acima.</p>
        </>
      )}
    </div>
  )
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">Gerencie os dados e prontuários dos pacientes</p>
        </div>

        {/* Badge de contagem */}
        {!carregando && !erro && (
          <div className="bg-white dark:bg-slate-800 px-5 py-3 rounded-2xl border border-slate-200 dark:border-slate-700/50 flex items-center gap-3 shadow-sm">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-700 dark:text-slate-400 uppercase tracking-wider">Total</p>
              <p className="text-xl font-extrabold text-slate-800 dark:text-slate-100">{pacientes.length}</p>
            </div>
          </div>
        )}
      </div>
      ...
      {/* ── Cadastro Rápido ─────────────────────────────────────────────────── */}
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700/50 mb-8 shadow-sm">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700/50 shadow-sm overflow-hidden">
        ...
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2 shrink-0">
          ...
          <div className="relative w-full sm:w-72">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-700 dark:text-slate-400 pointer-events-none" />
            <input
              type="text"
            ...
            {busca && (
              <button
                onClick={() => setBusca('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-700 dark:text-slate-400 hover:text-slate-600 dark:text-slate-300 transition-colors"
              >
                <X className="h-4 w-4" />
              </button>
            )}
        ...
        {!erro && (
          <div className="hidden sm:block overflow-x-auto w-full">
            <table className="w-full text-left border-collapse min-w-[640px]">
              <thead>
                <tr className="bg-white dark:bg-slate-900/40 border-b border-slate-200 dark:border-slate-700/50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-600 dark:text-slate-400">#{p.id.substring(0, 8)}</div>
                          </div>
                        </div>
                      </td>
                      ...
                      >
                        <div className="flex justify-end gap-2">
                          <button
                            ...
                            className="text-slate-500 hover:text-slate-600 dark:text-slate-300 bg-slate-700/30 hover:bg-slate-700/50 border border-slate-200 dark:border-slate-700/50 px-2 py-1.5 rounded-lg text-xs font-semibold transition-all inline-flex items-center gap-1.5 active:scale-95"
                          >
                ...
                >
                  ...
                  <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      className="text-slate-500 bg-slate-700/30 border border-slate-200 dark:border-slate-700/50 p-2 rounded-xl text-xs font-bold transition-all active:scale-[0.98]"
                    >
        ...
        {!carregando && !erro && pacientesFiltrados.length > 0 && (
          <div className="px-6 py-3 border-t border-slate-200 dark:border-slate-700/50 flex items-center justify-between">
