    ...
    <div className="flex flex-col h-full bg-slate-50 overflow-y-auto">
      <header className="p-8 bg-white border-b border-slate-200 shadow-sm shrink-0">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Régua de Cobrança e Lembretes</h1>
            <p className="text-sm font-semibold text-slate-500 mt-1">Automações de comunicação com pacientes via WhatsApp, SMS e E-mail</p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 p-8 max-w-7xl mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            ...
            <p className="text-2xl font-extrabold text-slate-800">{ativos}</p>
            <p className="text-xs font-semibold text-slate-500 mt-1">Réguas Ativas</p>
          </div>
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div className="p-2.5 bg-emerald-50 text-emerald-600 rounded-xl w-fit mb-3"><CheckCircle2 className="h-5 w-5" /></div>
            <p className="text-2xl font-extrabold text-slate-800">{enviados}</p>
            <p className="text-xs font-semibold text-slate-500 mt-1">Enviados Hoje</p>
          </div>
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            <div className="p-2.5 bg-red-50 text-red-600 rounded-xl w-fit mb-3"><AlertCircle className="h-5 w-5" /></div>
            <p className="text-2xl font-extrabold text-slate-800">{inadimplentes.length}</p>
            <p className="text-xs font-semibold text-slate-500 mt-1">Inadimplentes</p>
          </div>
          <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            ...
            <p className="text-xs font-semibold text-slate-500 mt-1">Total Inadimplente</p>
          </div>
        </div>

        {/* Tabs */}
        <div className="flex gap-1 bg-white rounded-2xl border border-slate-200 p-1.5 shadow-sm w-fit">
          {([['regras', 'Réguas de Comunicação'], ['logs', 'Log de Envios'], ['inadimplentes', 'Inadimplentes']] as const).map(([tab, label]) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === tab ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'}`}>{label}</button>
          ))}
        </div>
        ...
        {activeTab === 'regras' && (
          <div className="space-y-4">
            {regras.map(regra => {
              ...
              return (
                <div key={regra.id} className={`bg-white rounded-2xl border shadow-sm p-6 transition-all ${regra.ativa ? 'border-slate-200' : 'border-slate-100 opacity-60'}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        <h3 className="font-bold text-slate-800">{regra.nome}</h3>
                      ...
                      <p className="text-xs text-slate-500 font-semibold mb-3"><Clock className="h-3.5 w-3.5 inline mr-1" />{regra.antecedencia}</p>
                      <p className="text-sm text-slate-600 bg-slate-50 rounded-xl p-3 font-medium italic">"{regra.mensagem}"</p>
                    </div>
                  ...
                  {logs.map(l => {
                    ...
                    return (
                      <tr key={l.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-5 py-3.5 font-bold text-slate-800">{l.paciente}</td>
                        ...
                        <td className="px-5 py-3.5 text-slate-500">{l.dataEnvio}</td>
                        <td className="px-5 py-3.5 text-slate-500">{l.horario}</td>
        ...
        {activeTab === 'inadimplentes' && (
          <div className="space-y-4">
            {inadimplentes.map((p, i) => (
              <div key={i} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800">{p.paciente}</h4>
                    <p className="text-xs text-slate-500">{p.telefone} · {p.diasAtraso} dias de atraso</p>
                  </div>
                </div>
