      ...
      <div className="max-w-6xl mx-auto w-full px-6 py-12">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="text-center max-w-2xl mx-auto mb-12">
              ...
              <div className="inline-flex bg-slate-200/60 p-1 rounded-2xl items-center shadow-inner">
                <button 
                  ...
                  className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${billingCycle === 'monthly' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Mensal
                </button>
                <button 
                  onClick={() => setBillingCycle('annual')}
                  className={`px-6 py-2 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${billingCycle === 'annual' ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Anual
            ...
            <div className="grid md:grid-cols-3 gap-8">
              {plans.map((plan) => (
                <div 
                  key={plan.id}
                  className={`bg-white rounded-[32px] p-8 flex flex-col relative transition-all duration-300 hover:shadow-xl ${plan.popular ? 'ring-2 ring-blue-500 shadow-[0_10px_40px_-10px_rgba(37,99,235,0.15)] scale-105 z-10' : 'border border-slate-200 shadow-sm'}`}
                >
                  {plan.popular && (
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-slate-500 text-sm font-medium min-h-[40px]">{plan.description}</p>
                  </div>
                  ...
                  <div className="flex-1">
                    <h4 className="text-sm font-bold text-slate-800 mb-4 uppercase tracking-wider">O que está incluso:</h4>
        ...
        {step === 2 && (
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-right-8 duration-500">
            <button 
              onClick={() => setStep(1)}
              className="text-slate-500 hover:text-slate-800 font-bold text-sm mb-6 flex items-center gap-2 transition-colors"
            >
              &larr; Voltar para os planos
            </button>
            
            <div className="bg-white rounded-[32px] p-8 md:p-12 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] border border-slate-200">
              <div className="grid md:grid-cols-2 gap-12">
                
                {/* Formulário de Checkout */}
                <div>
                  <h2 className="text-2xl font-extrabold text-slate-800 mb-6 tracking-tight">Dados de Pagamento</h2>
                  
                  <div className="space-y-5">
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-1.5">Nome no Cartão</label>
                      <input 
                        type="text" 
                        placeholder="Como impresso no cartão"
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all"
                      />
                    </div>
                    
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-1.5">Número do Cartão</label>
                      <div className="relative">
                        <input 
                          type="text" 
                          placeholder="0000 0000 0000 0000"
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-4 pr-10 py-3 text-sm font-medium text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all"
                        />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-bold text-slate-700 mb-1.5">Validade</label>
                        <input 
                          type="text" 
                          placeholder="MM/AA"
                          className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-medium text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all"
                ...
                {/* Resumo da Compra */}
                <div className="bg-slate-50 p-6 md:p-8 rounded-[24px] border border-slate-200 h-fit">
                  <h3 className="text-lg font-extrabold text-slate-800 mb-6 border-b border-slate-200 pb-4">Resumo da Assinatura</h3>
                  
                  <div className="flex justify-between items-center mb-4">
                    <div>
                      <p className="font-bold text-slate-800 text-lg">Plano {selectedPlan.name}</p>
                      <p className="text-sm font-semibold text-slate-500">Cobrança {billingCycle === 'monthly' ? 'Mensal' : 'Anual'}</p>
                    </div>
                  ...
                  <div className="flex justify-between items-center mb-6">
                    <span className="font-bold text-slate-600">Total hoje</span>
        ...
        {step === 3 && (
          <div className="max-w-xl mx-auto text-center py-20 animate-in zoom-in-95 duration-500">
            ...
            <h1 className="text-4xl font-extrabold text-slate-800 mb-4 tracking-tight">Assinatura Confirmada!</h1>
